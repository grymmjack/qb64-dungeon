...
' MgOk, not Ok. A two-letter public symbol is a collision waiting to happen and
' this one duly collided: DICE3D's _GEO.BM has a local named `ok`, QB64
' identifiers are case-insensitive, and pulling the vendored module into a
' prototype that defined `SUB Ok` failed to compile with "Name already in use".
