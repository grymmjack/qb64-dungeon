#!/usr/bin/env bash
# drive-settings.sh <out.png> — intro -> menu -> SETTINGS (option 5), capture.
set -uo pipefail
OUT="$1"
...
key s 0.4; key s 0.4; key s 0.4; key s 0.4   # menu 1 -> 5 (SETTINGS)
key Return 1.0      # open SETTINGS
key s 0.5           # move highlight to Sound FX
sleep 0.3
