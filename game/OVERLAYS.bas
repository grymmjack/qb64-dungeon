...
SUB Game_RenderOverlays
    DrawBoardOverlays           ' data-driven art on the board (torches, and anything else)
    DrawTombstones
    DrawChamberGraves
    DrawEntities
...
' ----------------------------------------------------------------------------
'  BOARD OVERLAYS -- art placed on the board from a data file.
'
'  Drawn FIRST, under everything else: a torch on the wall belongs behind the
'  tombstones, the monsters and the player token, not in front of them.
'
'  An animated .gif works here with no special handling at all, because Sprite&
'  animates GIFs and this draws through Sprite& like every other sprite in the
'  game. That is the whole reason the animation lives in ARTPACK rather than in
'  any one call site.
' ----------------------------------------------------------------------------
SUB LoadBoardOverlays
    DIM i AS INTEGER, nm AS STRING

    OVL_N = 0
    ' the LOGICAL path: ReadDataFile routes it through the data pack itself.
    ReadDataFile "assets/data/overlays.txt"
