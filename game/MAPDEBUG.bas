...
SUB MdLegend
    ...
    _PRINTSTRING (28 * CW, CH), "[0] art  [A] all  [N] none  [-/=] tint " + LTRIM$(STR$(MD_ALPHA)) + "  [click/arrows] cell  [E] event  [O] overlay  [C] chamber  [S] shot  [ESC]"
END SUB
...
SUB DumpMapDebugShot (mask AS STRING, outp AS STRING)
    ...
    IF INSTR(LCASE$(COMMAND$), "chamber") > 0 THEN MD_SX = 24: MD_SY = 12: MD_RC1X = 13: MD_RC1Y = 7: MD_RC_SET = -1: MD_ON(6) = TRUE
    MdDraw
...
' ============================================================================
'  PLACEMENT -- the overlay and chamber-rectangle editors.
'
'  These are not separate tools. They live inside the map debugger because it
'  already owns cell picking, the board render and the layer tints -- and
'  because the check on a placement IS the layer view: an overlay is right when
'  it sits on the cell you meant, and a chamber rect is right when layer 6
'  tints the hall and not the corridor beside it. A standalone editor would
'  place the thing and then send you somewhere else to find out.
'
'  Both WRITE by appending a row of text, and both RELOAD immediately, so the
'  file stays exactly as hand-editable as it was and the screen shows the
'  consequence rather than the intention. Deleting goes through the data
'  editor's raw-line machinery (DeLoad / DeDeleteRow / DeSave), which is what
'  keeps the file's comments -- the format's only documentation -- intact.
'
'  Each panel's DRAWING is a separate SUB from its input loop, the same split
'  the event panel uses: a screen that exists only inside its own loop can be
'  checked by nothing but a person looking at it.
' ============================================================================

'--- the art a board overlay can name. Scanned from the cut-scene art dirs,
'    pack first then default, because that is where art meant to be composited
'    onto something already lives (Game_CutArtPath$ looks there first too). ---
SUB MdScanArt
    MD_PL_N = 0
    MdScanArtDir "assets/cutscenes/" + _TRIM$(opt_datapack) + "/art/"
    IF LCASE$(_TRIM$(opt_datapack)) <> "default" THEN MdScanArtDir "assets/cutscenes/default/art/"
...
'--- The art is drawn AT the cell while you aim, with the same centre-anchoring
'    DrawBoardOverlays uses, so what you are looking at is what the row will
'    produce -- not a marker standing in for it. ---
SUB MdOverlayPaint
    ...
    _PRINTSTRING (4 * CW, (py + 4) * CH), "cell   " + LTRIM$(STR$(MD_SX)) + "," + LTRIM$(STR$(MD_SY)) + "   level " + LTRIM$(STR$(SECTORAT(MD_SX, MD_SY)))
...
'--- scale moves through the values people actually use, not by a fixed step: a
'    torch wants 0.5 and a mural wants 3, and stepping 0.1 at a time between
'    them is thirty keypresses ---
...
'--- a SINGLE without BASIC's leading space or trailing zeroes ---
...
SUB MdWriteOverlay
    ...
    row = LTRIM$(STR$(SECTORAT(MD_SX, MD_SY))) + " | " + LTRIM$(STR$(MD_SX)) + " | " + LTRIM$(STR$(MD_SY))
    ...
    PRINT #f, row
    CLOSE #f
    LoadBoardOverlays                       ' show the consequence, not the intention
    MD_MSG = "wrote: " + row
END SUB

'--- delete through the data editor's raw-line machinery, so the file's
'    comments survive: they are the only documentation the format has ---
...
'--- the selected pack's copy of a table, falling back to default/ ---
FUNCTION MdDataPath$ (nm AS STRING)
    DIM p AS STRING
    p = "assets/data/" + _TRIM$(opt_datapack) + "/" + nm
    IF _FILEEXISTS(p) THEN MdDataPath$ = p ELSE MdDataPath$ = "assets/data/default/" + nm
END FUNCTION

' ----------------------------------------------------------------------------
'  Chamber rectangles
'
'  A chamber is a bounding box in cells, and EVERY walkable cell inside it
'  becomes a trigger for that hall -- so a box one column too wide swallows the
'  corridor beside it and the player fights the Armory's monsters in a hallway.
'  That is exactly the mistake the layer view makes obvious: place the rect,
'  DetectChambers re-runs, and layer 6 tints what it actually claimed.
' ----------------------------------------------------------------------------
SUB MdPlaceChamber
    DIM k AS STRING, done AS INTEGER
    ...
    DO
        _LIMIT 60
        MD_ON(6) = TRUE                     ' you cannot judge a rect without the layer it feeds
        MdDraw
...
'--- draws the rect being dragged and reports the corners it will write. The
'    corner values are returned, so the writer and the display can never
'    disagree about which rectangle you were looking at. ---
...
SUB MdWriteChamber (x1 AS INTEGER, y1 AS INTEGER, x2 AS INTEGER, y2 AS INTEGER)
    ...
    DetectChambers
    MD_MSG = "wrote: " + row + "   (chambers re-derived)"
END SUB
...
'--- a one-line text prompt. Deliberately its own tiny loop rather than the
'    game's PromptName$: that one is styled for a player naming a hero and
'    lives beside hot-seat seat handling. ---
